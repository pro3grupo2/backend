...
const verificar_JWT = (req, res, next) => {
    ...
    if (!authorization) return bad_response(res, 401, new Error(auth_errors.TOKEN_NEEDED))

    const [_, token] = authorization.split(" ")
    const data = auth_service.verificar_JWT(token)

    console.log(data)
    if (!data) return bad_response(res, 401, new Error(`${auth_errors.TOKEN_EXPIRED_OR_INVALID} : ${token}`))
...
const is_administrador = (req, res, next) => {
    if (req.JWT.rol !== "coordinador") return bad_response(res, 401, new Error(`${auth_errors.NOT_ADMININSTRADOR} : ${req.JWT.correo}`))
