...
const signin = [
    ...
    body('password', 'Type: String, Max-Length: 200, Hint: La contraseña debe tener al menos 8 caracteres, 1 mayúscula, 1 número y 1 carácter especial').exists().notEmpty().isString().isLength({max: 200}).custom(value => /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/.test(value)),
    validate
]
...
const signup = [
    ...
    body('promocion', 'Type: Year (YYYY)').exists().notEmpty().toInt().optional(),
    validate
]
